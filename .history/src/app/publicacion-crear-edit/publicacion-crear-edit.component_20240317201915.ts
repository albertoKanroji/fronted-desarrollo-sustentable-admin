...
export class PublicacionCrearEditComponent implements OnInit {
  ...
  async obtenerCategorias(): Promise<Categoria[]> { // Return type is Categoria[]
    this.cargandoCategorias = true;
    const categorias = await this.http
      .get<Categorias>("http://127.0.0.1:8000/api/v1/categorias/")
      .toPromise();
    this.categorias = categorias;
    this.cargandoCategorias = false;
    return categorias; // Explicitly return the fetched categories
  }
